# Case Studies / Dự án tiêu biểu
---

## Aethir
- **Client:** Aethir
- **Project Link:** 
https://aethir.com/
- **Collaboration Type:** MVP Development
- **Detailed Description:**
  - Aethir is a decentralized cloud computing platform that aggregates GPU power from enterprises, data centers, and individuals worldwide to provide GPU-as-a-Service for AI, gaming, and high-performance computing applications. Aethir reduces costs, increases scalability, and eliminates the centralization risks of traditional cloud services.
- **Challenges:**
  - Aethir had to solve the problem of high GPU costs, limited scalability, and centralization risks when deploying large-scale AI.
- **Services Provided by Cyberk:** [To be added]
- **Results & Achievements:**
  - Aethir enabled partners like OpenLedger to save up to 99% in costs, run thousands of AI models on a single GPU, and scale their global GPU network in just two weeks.
- **Image:** 
https://drive.google.com/file/d/1Byzq2meS4JpCz7O7K2-10yjp6EWQQqs8/view

https://docs.aethir.com/~gitbook/image?url=https%3A%2F%2F3028335560-files.gitbook.io%2F%7E%2Ffiles%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252FlJdZs7NyMJ6Ewm4U1eRP%252Fuploads%252Fnbv7YhrnxLY9ZrdtBwqc%252FHow%2520Aethir%2520Works.png%3Falt%3Dmedia%26token%3D14e5792b-9dc9-4844-915a-9f84faec19e0&width=768&dpr=4&quality=100&sign=d832f6b0&sv=2

- **Khách hàng:** Aethir
- **Liên kết dự án:** 
https://aethir.com/
- **Hình thức hợp tác:** MVP Development
- **Mô tả chi tiết:**
  - Aethir là nền tảng điện toán đám mây phi tập trung, tập hợp sức mạnh GPU từ các doanh nghiệp, trung tâm dữ liệu và cá nhân trên toàn cầu để cung cấp dịch vụ GPU-as-a-Service cho AI, gaming và các ứng dụng tính toán hiệu năng cao. Aethir giúp giảm chi phí, tăng khả năng mở rộng và loại bỏ rủi ro tập trung của các dịch vụ cloud truyền thống.
- **Thử thách:**
  - Aethir phải giải quyết bài toán chi phí GPU cao, khả năng mở rộng hạn chế và rủi ro tập trung khi triển khai AI quy mô lớn.
- **Dịch vụ Cyberk cung cấp:** [Bổ sung]
- **Kết quả & Thành tựu:**
  - Aethir giúp các đối tác như OpenLedger tiết kiệm tới 99% chi phí, vận hành hàng nghìn mô hình AI trên một GPU, và mở rộng mạng lưới GPU toàn cầu chỉ trong 2 tuần.
- **Hình ảnh:** 
https://drive.google.com/file/d/1Byzq2meS4JpCz7O7K2-10yjp6EWQQqs8/view

https://docs.aethir.com/~gitbook/image?url=https%3A%2F%2F3028335560-files.gitbook.io%2F%7E%2Ffiles%2Fv0%2Fb%2Fgitbook-x-prod.appspot.com%2Fo%2Fspaces%252FlJdZs7NyMJ6Ewm4U1eRP%252Fuploads%252Fnbv7YhrnxLY9ZrdtBwqc%252FHow%2520Aethir%2520Works.png%3Falt%3Dmedia%26token%3D14e5792b-9dc9-4844-915a-9f84faec19e0&width=768&dpr=4&quality=100&sign=d832f6b0&sv=2

---

## Coinseeker
- **Client:** Coinseeker
- **Project Link:** https://coinseeker.co/
- **Collaboration Type:** Dedicated Team
- **Detailed Description:**
  - Coinseeker is a blockchain data analytics platform that helps users discover new investment opportunities, track market trends, and manage digital asset portfolios intelligently.
- **Challenges:**
  - Building a real-time blockchain data analytics system, handling large data volumes, and ensuring security and transparency for users.
- **Services Provided by Cyberk:** [To be added]
- **Results & Achievements:**
  - Coinseeker was acquired by Titan Lab for $30M in January 2025, becoming one of the most notable M&A deals in blockchain analytics.
- **Image:**

https://drive.google.com/file/d/1wPsdcltJ7ymLLS6VDacJcIkAwAWp1qT8/view?usp=sharing

- **Khách hàng:** Coinseeker
- **Liên kết dự án:** https://coinseeker.co/
- **Hình thức hợp tác:** Dedicated Team
- **Mô tả chi tiết:**
  - Coinseeker là nền tảng phân tích dữ liệu blockchain, giúp người dùng phát hiện cơ hội đầu tư mới, theo dõi xu hướng thị trường và quản lý danh mục tài sản số một cách thông minh.
- **Thử thách:**
  - Xây dựng hệ thống phân tích dữ liệu blockchain real-time, xử lý khối lượng lớn dữ liệu và đảm bảo bảo mật, minh bạch cho người dùng.
- **Dịch vụ Cyberk cung cấp:** [Bổ sung]
- **Kết quả & Thành tựu:**
  - Coinseeker được Titan Lab mua lại với giá trị 30 triệu đô vào tháng 1/2025, trở thành một trong những thương vụ M&A nổi bật trong lĩnh vực blockchain analytics.
- **Hình ảnh:** 

https://drive.google.com/file/d/1wPsdcltJ7ymLLS6VDacJcIkAwAWp1qT8/view?usp=sharing

---

## Helix
- **Client:** Helix
- **Project Link:** 
https://www.helixfi.io/
- **Collaboration Type:** MVP Development
- **Detailed Description:**
  - Helix is a multi-strategy RWA (Real World Asset) DeFi platform powered by the USHD stablecoin, offering institutional-grade access to real-world assets, sustainable yield, and growth opportunities. Helix provides a full-stack, omni-chain ecosystem for seamless DeFi composability, backed by U.S. T-Bills and top-rated money market instruments, and recognized as an industry leader in RWA finance. [[helixfi.io](https://www.helixfi.io/)]
- **Challenges:**
  - Delivering institutional-grade transparency, security, and risk protection for on-chain finance, while enabling seamless integration across DeFi protocols and ecosystems.
- **Services Provided by Cyberk:** [To be added]
- **Results & Achievements:**
  - Helix has facilitated over $500M in off-chain deal volume, closed 400+ deals with zero defaults, and is backed by leading fintech investors and partners.
- **Image:**

https://drive.google.com/file/d/1m4nTr50mxFjUKwL61lQjRJ4S7laE57SD/view?usp=sharing

- **Khách hàng:** Helix
- **Liên kết dự án:** 
https://www.helixfi.io/
- **Hình thức hợp tác:** MVP Development
- **Mô tả chi tiết:**
  - Helix là nền tảng DeFi đa chiến lược về tài sản thực (RWA) được vận hành bởi stablecoin USHD, cung cấp quyền truy cập cấp tổ chức vào tài sản thực, lợi suất bền vững và cơ hội tăng trưởng. Helix mang đến hệ sinh thái full-stack, đa chuỗi cho DeFi, được bảo chứng bởi trái phiếu chính phủ Mỹ và các công cụ thị trường tiền tệ hàng đầu, được công nhận là đơn vị dẫn đầu trong lĩnh vực tài chính RWA. [[helixfi.io](https://www.helixfi.io/)]
- **Thử thách:**
  - Đảm bảo minh bạch, bảo mật và quản trị rủi ro cấp tổ chức cho tài chính on-chain, đồng thời tích hợp liền mạch với các giao thức và hệ sinh thái DeFi.
- **Dịch vụ Cyberk cung cấp:** [Bổ sung]
- **Kết quả & Thành tựu:**
  - Helix đã xử lý hơn 500 triệu đô giao dịch ngoài chuỗi, hoàn thành hơn 400 thương vụ mà không có khoản vỡ nợ nào, và được hậu thuẫn bởi các nhà đầu tư, đối tác fintech hàng đầu.
- **Hình ảnh:**

https://drive.google.com/file/d/1m4nTr50mxFjUKwL61lQjRJ4S7laE57SD/view?usp=sharing

---

## Oracler
- **Client:** Oracler
- **Project Link:** [To be added]
- **Collaboration Type:** Dedicated Team
- **Detailed Description:**
  - Oracler is a decentralized oracle solution that connects off-chain data to smart contracts on the blockchain, ensuring authenticity and transparency for DeFi, GameFi, and other applications.
- **Challenges:**
  - Ensuring reliability, security, and fast data updates for smart contracts, while preventing data manipulation attacks.
- **Services Provided by Cyberk:** [To be added]
- **Results & Achievements:**
  - Oracler has been successfully integrated with major DeFi projects, handling millions of data requests daily and raising the security standards for the blockchain ecosystem.
- **Image:** [To be added]

**Dịch tiếng Việt:**
- **Khách hàng:** Oracler
- **Liên kết dự án:** [Bổ sung]
- **Hình thức hợp tác:** Dedicated Team
- **Mô tả chi tiết:**
  - Oracler là dự án cung cấp giải pháp oracle phi tập trung, kết nối dữ liệu ngoài chuỗi (off-chain) với các hợp đồng thông minh trên blockchain, đảm bảo tính xác thực và minh bạch cho các ứng dụng DeFi, GameFi, v.v.
- **Thử thách:**
  - Đảm bảo độ tin cậy, bảo mật và tốc độ cập nhật dữ liệu cho các hợp đồng thông minh, đồng thời chống lại các cuộc tấn công thao túng dữ liệu.
- **Dịch vụ Cyberk cung cấp:** [Bổ sung]
- **Kết quả & Thành tựu:**
  - Oracler đã tích hợp thành công với nhiều dự án DeFi lớn, xử lý hàng triệu request dữ liệu mỗi ngày và góp phần nâng cao tiêu chuẩn bảo mật cho hệ sinh thái blockchain.
- **Hình ảnh:** [Bổ sung]

--- 